...
class SliderComponent extends HTMLElement {
  ...
  update() {
    ...
    if (this.id && this.id.startsWith('GalleryThumbnails')) {
      const scrollTop = this.slider.scrollTop;
      const scrollHeight = this.slider.scrollHeight;
      const clientHeight = this.slider.clientHeight;
  ...
  onButtonClick(event) {
    ...
    const isVertical = this.id && this.id.startsWith('GalleryThumbnails');

   if (isVertical) {
      const firstItem = this.slider.querySelector('.thumbnail-list__item');
      if (firstItem) {
        const style = window.getComputedStyle(firstItem);
        const marginBottom = parseFloat(style.marginBottom) || 0;
        const gap = parseFloat(window.getComputedStyle(this.slider).gap) || 8;
        const scrollAmount = firstItem.offsetHeight + gap;
        this.slider.scrollBy({
          top: event.currentTarget.name === 'next' ? scrollAmount : -scrollAmount,
          behavior: 'smooth'
        });
      }
      return;
